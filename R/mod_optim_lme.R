          ...
          style = "display:inline-block; vertical-align:top; margin-right:10px;",
            wellPanel(
              ...
              p(tags$b(name_with_info(
                "Descriptives",
                "The data simulated by the descriptives module, matching means and standard deviations of the data in wide format."
              ))),
              ...
              p(tags$b(name_with_info(
                "Correlations",
                "The correlation matrix containing the upper triangle of the reported bivariate correlations of the data in long format. If a value is not available, the cell is supposed to be blank."
              ))),
              div(
              ...
              p(tags$b(name_with_info(
                "Regression Table",
                "The reported unstandardized regression coefficients (Est.) and their standard erorrs (SE). If a value is not available, the cell is supposed to be blank."
              ))),
              div(
            ...
            div(
              ...
              numericInput(ns("max_iter"), name_with_info(
                "Max Iterations",
                "The maximum number of iterations the algorithm will run each time it restarts and for each variable."), 5e3,   min = 10,    step = 100),
              ...
              numericInput(ns("hill_climbs"), name_with_info(
                "Hill Climbs",
                "The number of hill climbing iterations for further refinement ."), 1,   min = 1,    step = 100),
...
#'
#' @noRd
mod_optim_lme_server <- function(id, root_session){
  moduleServer(id, function(input, output, session){
    ...
    observeEvent({
      ...
      input$corr_table,
      input$lme_outcome,
      input$lme_predictors,
      input$lme_interactions,
      input$tolerance,
      input$max_iter,
      input$init_temp,
      input$cooling_rate,
    ...
    simulated_data <- reactive({
      root_session$userData$lme_data()
    })
    ...
    output$var_names <- renderUI({
      ...
      if (is.data.frame(df) &&
          ncol(df) >= 3 &&
          length(var_time_cols) >= 2) {
        long <- wide_to_long(cbind(ID = seq_len(nrow(df)), df))
        long <- long[ , !(names(long) %in% c("ID","time")) ]
        tagList(
          tags$p("Data available!"),
          tags$p("Variables: ",  paste(colnames(long), collapse = ", "))
        )
        } else {
          tagList(
    ...
    observe({
      ...
      req(ncol(df) >= 3 && length(var_time_cols) >= 2,
          message = "Need at least two var_time columns named like V1_1, V1_2.")
    ...
    observeEvent(input$lme_outcome, {
      df <- simulated_data()
      if (!is.data.frame(df) || ncol(df) == 0) return(NULL)
      df <- wide_to_long(cbind(ID = seq_len(nrow(df)), df))
      df <- df[ , !(names(df) %in% c("ID","time")) ]
      out_var  <- input$lme_outcome
    ...
    observeEvent(input$lme_predictors, {
      sel_preds <- input$lme_predictors
    ...
    lme_formula_reactive <- reactive({
      out_var <- input$lme_outcome
      preds   <- input$lme_predictors
      inters  <- input$lme_interactions
      ...
      stats::as.formula(paste(out_var, "~", rhs, " + (1|ID)"))
    })

    output$lme_formula <- renderText({
    ...
    corr_df_reactive <- reactive({
      ...
      colnames(mat) <- vars
      if (length(vars) == 4) {
        mat[upper.tri(mat)] <-  c(0.02,  0.00,  0.00, -0.13, -0.19 , 0.38)
      }
    ...
    output$corr_table <- renderRHandsontable({
      ...
      tbl <- rhandsontable(df_corr, rowHeaders = colnames(df_corr), width = "100%", height = "200px") %>%
        ...
        hot_col(col = colnames(df_corr), format = "0.00")
      n <- ncol(df_corr)
      for (i in seq_len(n)) {
        for (j in seq_len(n)) {
          if (i >= j) {
    ...
    coef_df_reactive <- reactive({
      ...
      if (p == 5) {
        mat[1, ] <- c(256.50, -0.36, -12.70, 42.34, 28.36)
        mat[2, ] <- c(7.69,   0.23,   5.56,   2.11,   NA)
      }
      as.data.frame(mat, check.names = FALSE, stringsAsFactors = FALSE)
    })
    ...
    output$coef_table <- renderRHandsontable({
      ...
      tbl <- rhandsontable(df,
                           ...
                           height     = "200px") %>%
        hot_table(overflow = "visible") %>%
        hot_col(col = colnames(df), format = "0.00") %>%
        hot_cell(row      = 2,
                 col      = n,
                 readOnly = TRUE)
    ...
    output$weight_table <- renderRHandsontable({
      tbl <- rhandsontable(weight_df(), rowHeaders = NULL, width = "100%") %>%
        ...
        hot_col("Correlation",  title = "Correlation",  format = "0.00") %>%
        hot_col("Regression",    title = "Regression",    format = "0.00")
      tbl
    })
    ...
    observeEvent(input$estimate_weights, {
      ...
      target_reg   <- as.numeric(coef_df["Est.",     ])
      ses     <- as.numeric(coef_df["SE",       ])
      target_se <- ses[-length(ses)]
      reg_equation <- paste(deparse(lme_formula_reactive()), collapse = "")
    ...
    for (btn in c(
      "plot_error", "plot_error_ratio", "get_rmse", "get_rmse_parallel",
      "plot_summary", "plot_rmse", "plot_cooling", "run",
      "display_data", "download", "estimate_weights", "dataset_selector"
    )) {
      shinyjs::disable(btn)
    }

    observe({
      preds_ok <- length(input$lme_predictors) > 0
    ...
    observeEvent(input$run, {
      ...
      for (btn in c(
        ...
        "run", "plot_error", "plot_error_ratio", "get_rmse",
        "get_rmse_parallel", "plot_summary", "plot_rmse", "plot_cooling",
        "display_data", "download", "dataset_selector", "plot_error",
        "plot_error_ratio", "get_rmse", "plot_summary", "plot_cooling",
